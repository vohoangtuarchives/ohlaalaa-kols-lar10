        ...
        <div class="col-md-4  mb-3">
            ...
            <select wire:model="city" name="city"  wire:click="city($event.target.value)" class="form-select">
                ...
                @foreach($cities as $city)
                    <option value="{{$city->id}}" @if((int)$customer->city == $city->id) selected @endif>{{$city->title}}</option>
                @endforeach
            </select>
            ...
            <select wire:model="district" name="district" wire:click="district($event.target.value)" class="form-select">
                ...
                @isset($districts)
                    @foreach($districts as $district)
                        <option value="{{$district->id}}"  @if((int) $customer->district == $district->id) selected @endif>{{$district->title}}</option>
                    @endforeach
                @endisset
            </select>
        ...
        <div class="col-md-4  mb-3">
            ...
            <select wire:model="ward" name="ward" class="form-select" wire:click="ward($event.target.value)">
                ...
                @foreach($wards as $ward)
                    <option value="{{$ward->id}}"  @if((int) $customer->ward == $ward->id) selected @endif>{{$ward->title}}</option>
                @endforeach
                @endisset
            </select>
